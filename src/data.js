// Wacode 2021 Schedule Data
export const appointments = [
  {
    title: "Check-in / CS Trivia",
    location: "Twitch",
    description:
      "As we prepare for the opening ceremony, join us on Twitch for some fun computer science trivia!",
    startDate: new Date(2021, 1, 27, 10, 30),
    endDate: new Date(2021, 1, 27, 11, 0),
    id: 0,
  },
  {
    title: "Opening Ceremony",
    location: "Twitch",
    description:
      "The opening ceremony! Get hyped for the event, learn about our sponsors, and get ready for the prompt reveal!",
    startDate: new Date(2021, 1, 27, 11, 0),
    endDate: new Date(2021, 1, 27, 11, 30),
    id: 1,
  },
  {
    title: "Team Building",
    location: "Discord",
    description:
      "Group up with fellow hackers and make teams if you don't already have one!",
    startDate: new Date(2021, 1, 27, 11, 30),
    endDate: new Date(2021, 1, 27, 12, 0),
    id: 2,
  },
  {
    title: "Intro to Git Worshop",
    location: "Zoom",
    description:
      "Learn about Git, the most common source control software in the world today. Topics include using Git to share code as a team as well as a basic introduction to the software as a whole.",
    startDate: new Date(2021, 1, 27, 12, 30),
    endDate: new Date(2021, 1, 27, 13, 30),
    id: 3,
  },
  {
    title: "Intro to Web Development Worshop",
    location: "Zoom",
    description:
      "Come for a brief introduction to HTML and CSS, the backbones of web design.",
    startDate: new Date(2021, 1, 27, 13, 30),
    endDate: new Date(2021, 1, 27, 14, 30),
    id: 4,
  },
  {
    title: "Mobile Development Worshop (featuring Credera)",
    location: "Zoom",
    description: "Description TBD.",
    startDate: new Date(2021, 1, 27, 15, 0),
    endDate: new Date(2021, 1, 27, 16, 0),
    id: 5,
  },
  {
    title: "AWS Worshop (featuring Amazon AWS)",
    location: "Zoom",
    description: "Description TBD.",
    startDate: new Date(2021, 1, 27, 16, 30),
    endDate: new Date(2021, 1, 27, 17, 30),
    id: 6,
  },
  {
    title: "Dinner Meet & Greet",
    location: "Discord",
    description: "Come chat with other hackers and share a meal!",
    startDate: new Date(2021, 1, 27, 18, 0),
    endDate: new Date(2021, 1, 27, 18, 30),
    id: 7,
  },
  {
    title: "Programming Contest",
    location: "Kattis/Discord",
    description:
      "Test your problem-solving abilities and data structure and algorithm knowledge to solve as many problems as you can! THe contest will take place on Kattis.",
    startDate: new Date(2021, 1, 27, 19, 30),
    endDate: new Date(2021, 1, 27, 22, 30),
    id: 8,
  },
  {
    title: "Jackbox Party Pack",
    location: "Discord/Twitch",
    description:
      "Play Jackbox Party Pack games with us! Possible games include Quiplash, Trivia Murder Party, Fibbage, TeeKO, etc.",
    startDate: new Date(2021, 1, 27, 18, 30),
    endDate: new Date(2021, 1, 27, 19, 30),
    id: 9,
  },
  {
    title: "Introduction to Machine Learning",
    location: "Zoom",
    description:
      "Come learn about some basic machine learning using Tensorflow and Python!",
    startDate: new Date(2021, 1, 27, 19, 30),
    endDate: new Date(2021, 1, 27, 20, 30),
    id: 10,
  },
  {
    title: "Jeopardy!",
    location: "Discord",
    description:
      "This... is... Jeopardy! Play with your team or with new teams to get the most points!",
    startDate: new Date(2021, 1, 27, 21, 0),
    endDate: new Date(2021, 1, 27, 22, 0),
    id: 11,
  },
  {
    title: "Bingo",
    location: "Discord",
    description:
      "Bring your blotters and game faces for a few fun games of Bingo!",
    startDate: new Date(2021, 1, 27, 22, 0),
    endDate: new Date(2021, 1, 27, 23, 0),
    id: 12,
  },
  {
    title: "Late Night Movie",
    location: "Discord",
    description: "Let's watch a movie together! We will be watching TBD.",
    startDate: new Date(2021, 1, 27, 23, 0),
    endDate: new Date(2021, 1, 28, 0, 30),
    id: 13,
  },
  {
    title: "Among Us",
    location: "Discord",
    description:
      "There is an imposter among us! Join us for a late night Among Us session.",
    startDate: new Date(2021, 1, 28, 0, 30),
    endDate: new Date(2021, 1, 28, 1, 30),
    id: 14,
  },
  {
    title: "Breakfast Meet & Greet",
    location: "Discord",
    description: "Come chat with other hackers and share a meal!",
    startDate: new Date(2021, 1, 28, 6, 0),
    endDate: new Date(2021, 1, 28, 6, 30),
    id: 15,
  },
  {
    title: "Submission & Presentation Workshop",
    location: "Zoom",
    description:
      "Come learn about the ins and outs of submitting your project and preparing for the final presentation.",
    startDate: new Date(2021, 1, 28, 6, 30),
    endDate: new Date(2021, 1, 28, 7, 30),
    id: 16,
  },
  {
    title: "Pictionary",
    location: "Discord",
    description:
      "Drawing and guessing is the nature of this game! Can you be the first to guess the drawing?",
    startDate: new Date(2021, 1, 28, 8, 0),
    endDate: new Date(2021, 1, 28, 9, 0),
    id: 17,
  },
  {
    title: "Presentation Practice",
    location: "Discord",
    description:
      "Pair up with another team to practice your presentations and get some constructive critism.",
    startDate: new Date(2021, 1, 28, 10, 0),
    endDate: new Date(2021, 1, 28, 11, 0),
    id: 18,
  },
  {
    title: "Judging Period",
    location: "Discord",
    description:
      "Give your presentations to our judges. Give it your best shot!",
    startDate: new Date(2021, 1, 28, 12, 0),
    endDate: new Date(2021, 1, 28, 13, 0),
    id: 19,
  },
  {
    title: "Submission Review Stream",
    location: "Twitch",
    description:
      "Tune in to see your fellow hackers' projects before the closing ceremony.",
    startDate: new Date(2021, 1, 28, 12, 30),
    endDate: new Date(2021, 1, 28, 13, 30),
    id: 20,
  },
  {
    title: "Lunch Meet & Greet",
    location: "Discord",
    description: "Come chat with other hackers and share a meal!",
    startDate: new Date(2021, 1, 28, 13, 0),
    endDate: new Date(2021, 1, 28, 13, 30),
    id: 21,
  },
  {
    title: "Closing Ceremony",
    location: "Twitch",
    description:
      "As we wrap up our event, tune in to hear some final words from our sponsors and find out if you won a prize!",
    startDate: new Date(2021, 1, 28, 13, 30),
    endDate: new Date(2021, 1, 28, 14, 0),
    id: 22,
  },
];

// Wacode 2020 Schedule Data
// export const appointments = [
//   {
//     title: "🧾 Sign-in & Registration",
//     location: "Cashion Lobby",
//     description:
//       "Table will be set up in the lobby where you can retreive your WiFi access key and free shirt!",
//     startDate: new Date(2020, 1, 22, 7, 45),
//     endDate: new Date(2020, 1, 22, 8, 15),
//     id: 0,
//   },
//   {
//     title: "☕ Light Breakfast",
//     location: "Cashion 5th Floor Corridor",
//     description:
//       "Light breakfast refreshments provided! Fresh fruit, donuts, hot coffee, and more!",
//     startDate: new Date(2020, 1, 22, 7, 45),
//     endDate: new Date(2020, 1, 22, 8, 45),
//     id: 1,
//   },
//   {
//     title: "🎆 Kick-Off Ceremony",
//     location: "Blume Conference Center",
//     description:
//       "Starting the day off with a quick run-down of what to expect, and giving credit where credit is due.",
//     startDate: new Date(2020, 1, 22, 8, 15),
//     endDate: new Date(2020, 1, 22, 8, 30),
//     id: 2,
//   },
//   {
//     title: "🤝 Team Matching Activities",
//     location: "Cashion 501",
//     description:
//       "Want to be on a team but don't have one? We'll help to match you with teammates of a similar skill-level!",
//     startDate: new Date(2020, 1, 22, 8, 30),
//     endDate: new Date(2020, 1, 22, 9, 0),
//     id: 3,
//   },
//   {
//     title: "📖 Web Dev Resource Sharing",
//     location: "Cashion 502",
//     description:
//       "New to programming and don't know where to start? We'll help you and your team out.",
//     startDate: new Date(2020, 1, 22, 8, 45),
//     endDate: new Date(2020, 1, 22, 9, 10),
//     id: 4,
//   },
//   {
//     title: "🧠 Intro to Design Thinking feat. Credera",
//     location: "Cashion 502",
//     description:
//       "In an immersive workshop presented by Credera, participants will learn how to build products that solve the RIGHT problems! Highly encouraged for all skill-levels!",
//     startDate: new Date(2020, 1, 22, 9, 15),
//     endDate: new Date(2020, 1, 22, 10, 45),
//     id: 5,
//   },
//   {
//     title: "🥪 Lunch",
//     location: "Cashion 5th Floor Corridor",
//     description:
//       "Grab some grub to re-fule your brain! Tons of options for you to chose from!",
//     startDate: new Date(2020, 1, 22, 11, 0),
//     endDate: new Date(2020, 1, 22, 12, 0),
//     id: 6,
//   },
//   {
//     title: "💻 Hacking Period",
//     location: "Blume Conference Center",
//     description:
//       "Grab a table for your team and hack away at your project. Mentors will be going around the room to provide insights and advice. Let us know if you need anything!",
//     startDate: new Date(2020, 1, 22, 8, 30),
//     endDate: new Date(2020, 1, 22, 20, 0),
//     id: 7,
//   },
//   {
//     title: "🧘 Afternoon Yoga w/ Billicarole!",
//     location: "Fountain Mall (Outside in front of Moody Library)",
//     description:
//       "Unwind your body and mind with a relaxing afternoon yoga session led by our very own Billicarole!",
//     startDate: new Date(2020, 1, 22, 12, 15),
//     endDate: new Date(2020, 1, 22, 12, 45),
//     id: 8,
//   },
//   {
//     title: "🎲 Rubik's Cube Competition",
//     location: "Cashion 501",
//     description:
//       "How fast can you solve a Rubik's Cube? It's just an algorithm! Prize available for the winner!",
//     startDate: new Date(2020, 1, 22, 13, 0),
//     endDate: new Date(2020, 1, 22, 13, 20),
//     id: 9,
//   },
//   {
//     title: "🤓 Computer Science Trivia!",
//     location: "Cashion 501",
//     description:
//       "So you think you know computer science and its rich history? Let's put that to the test! Prize available for the winner!",
//     startDate: new Date(2020, 1, 22, 13, 25),
//     endDate: new Date(2020, 1, 22, 13, 45),
//     id: 10,
//   },
//   {
//     title: "⏳ Leetcode Challenge!",
//     location: "Cashion 501",
//     description:
//       "Let's test your knowledge of Data Structures & Algorithms with a coding challenge! Prize available for the winner!",
//     startDate: new Date(2020, 1, 22, 14, 0),
//     endDate: new Date(2020, 1, 22, 14, 30),
//     id: 11,
//   },
//   {
//     title: "🏀 #1 Baylor vs. #3 Kansas Watch Party!",
//     location: "Cashion 502",
//     description:
//       "Cheer on our Baylor Bears as they fend to keep their #1 spot agains the Kansas Jayhawks! Game will be live-streamed on the projector for anyone who wants to take a break and watch.",
//     startDate: new Date(2020, 1, 22, 11, 0),
//     endDate: new Date(2020, 1, 22, 14, 45),
//     id: 12,
//   },
//   {
//     title: "🍲 Dinner",
//     location: "Cashion 5th Floor Corridor",
//     description:
//       "Last meal of the day! Get some great food to make that final push as we start to wrap up the day.",
//     startDate: new Date(2020, 1, 22, 17, 0),
//     endDate: new Date(2020, 1, 22, 18, 0),
//     id: 13,
//   },
//   {
//     title: "✈️ Paper Airplane Competition",
//     location: "Cashion 5th Floor Corridor",
//     description:
//       "Computer Scientists are Engineers, too! Let's put those skills to the test and see who can design the best paper airplane! Prize available for the winner!",
//     startDate: new Date(2020, 1, 22, 18, 30),
//     endDate: new Date(2020, 1, 22, 19, 0),
//     id: 14,
//   },
//   {
//     title: "🥤 Cup Stacking Competition",
//     location: "Cashion 501",
//     description:
//       "How high can you stack your tower? Team up with a partner and face off with others to see who can build the tallest cup tower in 60 seconds! Prizes available for the winners!",
//     startDate: new Date(2020, 1, 22, 15, 30),
//     endDate: new Date(2020, 1, 22, 16, 0),
//     id: 15,
//   },
//   {
//     title: "💾 Submit to DevPost & Finalize Presentations",
//     location: "DevPost link coming soon!",
//     description:
//       'We and many other hackathons use DevPost to better sort and document the wonderful projects our teams come up with throughout the day. You will simply create an account and create a project under our "Wacode 2020" hackathon page! Also take a chance to polish any slides or rehearse a demo before judges go around and make their final observations!',
//     startDate: new Date(2020, 1, 22, 20, 0),
//     endDate: new Date(2020, 1, 22, 20, 30),
//     id: 16,
//   },
//   {
//     title: "🕵️ Judging Period",
//     location: "Blume Conference Center",
//     description:
//       "Judges will go around the room to evaluate the projects you guys have come up with. We will be sure to keep in mind which bracket everyone is competing in.",
//     startDate: new Date(2020, 1, 22, 20, 30),
//     endDate: new Date(2020, 1, 22, 21, 15),
//     id: 17,
//   },
//   {
//     title: "🏆 Prize Presentations & Closing Remarks",
//     location: "Blume Conference Center",
//     description:
//       "We say our final words, present our hackathon winners, and say a massive thank you to our participants and sponsors! Thanks for making Wacode 2020 the best one yet! Everyone have a safe trip home!",
//     startDate: new Date(2020, 1, 22, 21, 15),
//     endDate: new Date(2020, 1, 22, 21, 45),
//     id: 18,
//   },
//   {
//     title: "🏆 Prize Presentations & Closing Remarks",
//     location: "Blume Conference Center",
//     description:
//       "We say our final words, present our hackathon winners, and say a massive thank you to our participants and sponsors! Thanks for making Wacode 2020 the best one yet! Everyone have a safe trip home!",
//     startDate: new Date(2020, 1, 22, 21, 15),
//     endDate: new Date(2020, 1, 22, 21, 30),
//     id: 18,
//   },
//   {
//     title: "🐋 Deployments using Kubernetes feat. Credera",
//     location: "Cashion 502",
//     description:
//       "Containerization is cool! Scaling your apps with Kubernetes is even cooler! Come learn the way of the whale with this amazing workshop led by our friends at Credera!",
//     startDate: new Date(2020, 1, 22, 16, 0),
//     endDate: new Date(2020, 1, 22, 17, 0),
//     id: 19,
//   },
// ];

// Wacode 2019 Schedule Data!
// Keep this here for the memories :'^)

// export const appointments = [
//   {
//     title: "Sign-in & Registration (Cashion Lobby)",
//     startDate: new Date(2019, 2, 2, 8, 0),
//     endDate: new Date(2019, 2, 2, 8, 45),
//     id: 0
//   },
//   {
//     title: "Kick-Off Ceremony (Blume)",
//     startDate: new Date(2019, 2, 2, 9, 0),
//     endDate: new Date(2019, 2, 2, 9, 30),
//     id: 1
//   },
//   {
//     title: "American Airlines Presents: Intro to React (Workshop Room)",
//     startDate: new Date(2019, 2, 2, 9, 30),
//     endDate: new Date(2019, 2, 2, 10, 30),
//     id: 2
//   },
//   {
//     title: "Brunch Served (5th Floor Corridor)",
//     startDate: new Date(2019, 2, 2, 10, 30),
//     endDate: new Date(2019, 2, 2, 11, 30),
//     id: 3
//   },
//   {
//     title: "Kahoot: Tech Trivia! (Activity Room)",
//     startDate: new Date(2019, 2, 2, 11, 0),
//     endDate: new Date(2019, 2, 2, 11, 30),
//     id: 4
//   },
//   {
//     title: "Pariveda Presets: Intro to Functional Programming (Workshop Room)",
//     startDate: new Date(2019, 2, 2, 11, 30),
//     endDate: new Date(2019, 2, 2, 12, 30),
//     id: 5
//   },
//   {
//     title: "Tech Tree Root Presents: Intro to iOS Development (Workshop Room)",
//     startDate: new Date(2019, 2, 2, 12, 45),
//     endDate: new Date(2019, 2, 2, 1, 15),
//     id: 6
//   },
//   {
//     title: "Rubik's Cube Competition! (Activity Room)",
//     startDate: new Date(2019, 2, 2, 13, 0),
//     endDate: new Date(2019, 2, 2, 13, 30),
//     id: 7
//   },
//   {
//     title: "Cerdera Presents: Intro to Serverless Architecture (Workshop Room)",
//     startDate: new Date(2019, 2, 2, 13, 30),
//     endDate: new Date(2019, 2, 2, 14, 30),
//     id: 8
//   },
//   {
//     title: "Dr. Pepper Hour! (5th Floor Corridor)",
//     startDate: new Date(2019, 2, 2, 14, 45),
//     endDate: new Date(2019, 2, 2, 15, 45),
//     id: 9
//   },
//   {
//     title: "Evening Yoga! (Activity Room)",
//     startDate: new Date(2019, 2, 2, 16, 0),
//     endDate: new Date(2019, 2, 2, 16, 30),
//     id: 10
//   },
//   {
//     title: "Dinner Served! (5th Floor Corridor)",
//     startDate: new Date(2019, 2, 2, 17, 0),
//     endDate: new Date(2019, 2, 2, 18, 0),
//     id: 11
//   },
//   {
//     title: "Cupstacking Competition! (Activity Room)",
//     startDate: new Date(2019, 2, 2, 18, 30),
//     endDate: new Date(2019, 2, 2, 19, 0),
//     id: 12
//   },
//   {
//     title: "Final Submissions! (DevPost)",
//     startDate: new Date(2019, 2, 2, 19, 45),
//     endDate: new Date(2019, 2, 2, 20, 15),
//     id: 13
//   },
//   {
//     title: "Judging Begins (Blume)",
//     startDate: new Date(2019, 2, 2, 20, 20),
//     endDate: new Date(2019, 2, 2, 21, 0),
//     id: 14
//   },
//   {
//     title: "Award Ceremony & Final Remarks (Blume)",
//     startDate: new Date(2019, 2, 2, 21, 5),
//     endDate: new Date(2019, 2, 2, 21, 35),
//     id: 15
//   }
// ];
